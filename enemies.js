...
function Eagle(x, y, type, difficulty) {
  ...
  this.update = function() {
    ...
    if (this.type === "walking") {
      ...
      if (this.position.x <= this.Lbarrier || this.position.x >= this.Rbarrier){
...
function checkEagleCollisions() {
  for (let eagle of eagles) {
    const collision = eagle.checkCollision(gameChar_x, gameChar_y, 30, 60)
    if (collision) {
      if (lives > 0) {
        handleDeath();
      }
